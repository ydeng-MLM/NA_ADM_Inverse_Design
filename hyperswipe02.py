...
if __name__ == '__main__':
    ...
    for kernel_first in conv_kernel_size_first_list:
        for kernel_second in conv_kernel_size_second_list:
            ...
            flags.conv_kernel_size[1] = kernel_first
            flags.conv_kernel_size[2] = kernel_second
            flags.conv_kernel_size[3] = kernel_second
            for reg_scale in reg_scale_list:
                flags.reg_scale = reg_scale
                for j in range(1,4):
                        flags.model_name = flags.data_set + "reg"+ str(flags.reg_scale) + "trial_"+str(j) + "_conv_kernel_swipe[" + str(kernel_first)+ "," +str(kernel_first)+ ","+ str(kernel_second) + "," + str(kernel_second) + "]"

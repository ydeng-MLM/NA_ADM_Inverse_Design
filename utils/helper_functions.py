...
# 14
def simulator_meta_material_ensemble(Xpred):
    """
    ...
    """
    np.savetxt('D:/Yang_MM_Absorber_ML/useless/Xpred.csv', Xpred)
    # Path has to be changed for pickel to unpickel the model from checkpoint file
    cwd = os.getcwd()
    print("Your are currenlty in folder", cwd)
    os.chdir('D:/Yang_MM_Absorber_ML/Backprop/ensemble_mm')
    print("Now you are in folder", os.getcwd())
    ensemble_predict_master('D:/Yang_MM_Absorber_ML/Backprop/ensemble_mm/models', 'D:/Yang_MM_Absorber_ML/useless/Xpred.csv')
    os.chdir(cwd)
    Ypred = np.loadtxt('D:/Yang_MM_Absorber_ML/useless/Ypred_ensemble.csv', delimiter=' ')
    return Ypred
